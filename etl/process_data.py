...
def set_primary_key_sessions(row, movies):
    ...
    """
    try:
        id_imdb = movies[movies["title"].str.lower() == row["title"].lower()][
            "id_imdb"
        ].values[0]
        if id_imdb:
            row["id_imdb"] = id_imdb
...
def create_sessions(sessions, filename):
    ...
    print(("-" * 10) + "{}".format(datetime_month_week_file) + ("-" * 10))
    ...
    if datetime_month_week_file <= "2013-01-1":
        header = [
            ...
            "admissions_total",
            "admissions_delta",
            "admissions_cinema_mean",
            "admissions_screen_mean",
            "amount_eur",
            "spectators",
        ]
    ...
    # Second Season
        ...
        datetime_month_week_file > "2013-01-1"
        and datetime_month_week_file <= "2014-05-3"
        and datetime_month_week_file != "2013-01-4"
        and datetime_month_week_file != "2013-12-5"
    ):
        header = [
            "rank",
            "title",
    ...
    # Fourth Season
        ...
        datetime_month_week_file > "2015-05-5"
        and datetime_month_week_file <= "2016-12-4"
    ):
        header = [
            "rank",
            "title",
...
def set_imdb_info(row):
    ...
    try:
        ...
        for feature in features_with_name:
            if feature in movie.keys():
                row[feature] = [
                    elem["name"] for elem in movie[feature] if "name" in elem.keys()
                ]
            else:
                row[feature] = []
    except:
...
def export_files_to_csv(sessions, movies, datetime_month_week_file):
    ...
    sessions.to_csv(
        processed_data_path + "/csv/sessions_{}.csv".format(datetime_month_week_file),
        index=False,
    )
    movies.to_csv(
        processed_data_path + "/csv/movies_{}.csv".format(datetime_month_week_file),
        index=False,
    )
...
def export_files_to_json(sessions, movies, datetime_month_week_file):
    ...
    with open(
        processed_data_path + "/json/sessions_{}.json".format(datetime_month_week_file),
        "w",
    ) as f:
    ...
    with open(
        processed_data_path + "/json/movies_{}.json".format(datetime_month_week_file),
        "w",
    ) as f:
        f.write(movies_json)
