...
processed_data_path = os.path.abspath(
    os.path.join(os.getcwd(), os.pardir, "data_geographical", "processed")
)
...
def generate_geographical_data(filename):
    ...
    df_current_year = df[['PROVINCIA',gross_colname_current_year,admissions_colname_current_year]].copy()
    df_current_year['year'] = year
    df_current_year.columns = ['province','gross_total','admissions_total','year']



    df_last_year = df[['PROVINCIA',gross_colname_last_year,admissions_colname_last_year]].copy()
